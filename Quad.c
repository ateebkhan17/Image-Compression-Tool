...
typedef struct quad {
  ...
  struct quad *left; // A pointer to the left child
  struct quad *right; // A pointer to its right child
} Quad;
...
Quad *BST_insert(Quad *root, Quad *new_node) {
  ...
  if(root == NULL){
    ...
    if(root->key == new_node->key){
      printf("Duplicate Quad (tx,ty,tx)=%d,%d, %d, was ignored\n", new_node->tx, new_node->ty, new_node->sx);
    ...
    if(root != NULL)
    {
      ...
      if(root->key > key){
        ...
        if(root->right == NULL){
          ...
          Quad *val = find_successor(root->right);
          tmp->key = val->key;
          tmp->h = val->h;
          tmp->tx = val->tx;
          tmp->ty = val->ty;
          tmp->w = val->w;
          tmp->wsplit = val->wsplit;
          tmp->right = BST_delete(tmp->right, val->tx, val->ty);
        }
      }
    }
  }
...
void BST_inorder(Quad *root, int depth) {
  ...
  printf("Depth=%d, key=%d, tx:ty (%d:%d), w=%d, h=%d, wsplit=%d\n", depth, \
  root->tx + (root->ty * root->sx), root->tx, root->ty, root->w, root->h, \
  root->wsplit);
...
void BST_preorder(Quad *root, int depth) {
  ...
  printf("Depth=%d, key=%d, tx:ty (%d:%d), w=%d, h=%d, wsplit=%d\n", \
  depth, root->tx + (root->ty * root->sx), root->tx, root->ty, root->w, \
  root->h, root->wsplit);
...
//helper functions for split_tree

///////////////////////////////////////////////////////////////////////////////

Quad *split_not_similar(Quad *new_root, Quad *root){
  if(new_root == NULL){
    if(root->wsplit == 0){
      new_root = new_Quad(root->tx, root->ty, root->w, (root->h)/2, 1, root->sx);
      Quad *a_quad = new_Quad(root->tx, root->ty + ((root->h)/2), root->w, (root->h)/2, 1, root->sx);
      BST_insert(new_root, a_quad);
    }else{
      new_root = new_Quad(root->tx, root->ty, (root->w)/2, root->h, 0, root->sx);
      Quad *b_quad = new_Quad(root->tx + ((root->w)/2), root->ty, (root->w)/2, root->h, 0, root->sx);
      BST_insert(new_root, b_quad);
    }
  }else{
    if(root->wsplit == 0){
      Quad *c_quad = new_Quad(root->tx, root->ty, root->w, (root->h)/2, 1, root->sx);
      BST_insert(new_root, c_quad);
      Quad *d_quad = new_Quad(root->tx, root->ty + ((root->h)/2), root->w,  (root->h)/2, 1, root->sx);
      BST_insert(new_root, d_quad);
    }else{
      Quad *e_quad = new_Quad(root->tx, root->ty, (root->w)/2, root->h, 0, root->sx);
      BST_insert(new_root, e_quad);
      Quad *f_quad = new_Quad(root->tx + ((root->w)/2), root->ty, (root->w)/2, root->h, 0, root->sx);
      BST_insert(new_root, f_quad);
    }
  }
  return new_root;
}

Quad *split_similar(Quad *new_root, Quad *root){
  if(new_root == NULL){
    new_root = new_Quad(root->tx, root->ty, root->w, root->h, root->wsplit, root->sx);
  }else{
    Quad *g_quad = new_Quad(root->tx, root->ty, root->w, root->h, root->wsplit, root->sx);
    BST_insert(new_root, g_quad);
  }
  return new_root;
}

Quad *split_tree_spliter(Image *im, Quad *root, int threshold, Quad *new_root){
  if(root == NULL){
    return root;
  }
  if(similar(im, root, threshold) == 1){
    new_root = split_similar(new_root, root);
  }else{
    new_root = split_not_similar(new_root, root);
  } 
  split_tree_spliter(im, root->left, threshold, new_root);
  split_tree_spliter(im, root->right, threshold, new_root);
  return new_root;
}
...
Quad *split_tree(Image *im, Quad *root, int threshold) {
   ...
   * TODO: Implement this function
   */
  Quad *new_split_root = NULL;
  new_split_root = split_tree_spliter(im, root, threshold, new_split_root);
  delete_BST(root);
  return new_split_root;
}
...
void drawOutline(Image *im, Quad *root, unsigned char col) {
  ...
  int i = root->tx;
  while(i < root->tx + root->w){
    im->data[i + ((root->ty) * root->sx)] = col;
    i++;
  }
  int k = root->tx;
  while(k < root->tx + root->w){
    im->data[k + ((root->ty + root->h-1) * root->sx)] = col;
    k++;
  }
  int j = root->ty;
  while(j < root->ty + root->h){
    im->data[(root->tx) + (j  * root->sx)] = col;
    j++;
  }
  int l = root->ty;
  while(l < root->ty + root->h){
    im->data[(root->tx + root->w-1) + (l * root->sx)] = col;
    l++;
  }
}
...
void save_Quad(Image *im, Quad *root) {
  ...
  int expected_colour = 0;
  expected_colour = get_colour(im, root);
  int x = root->tx;
  while(x < root->tx + root->w){
    int y = root->ty;
    while(y < root->ty + root->h){
      int index = x + (y * root->sx);
      im->data[index] = expected_colour;
      y++;
    }
    x++;
  }
